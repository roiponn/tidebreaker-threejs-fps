...
| V6 | Decals are offset quads, not projected — they will float on curved surfaces | Low-medium | `effects/DecalSystem.ts` |
...
| # | Issue | Severity | Where |
| --- | --- | --- | --- |
| G1 | A full playthrough (enemy fire → damage → death → extraction) was never exercised end to end; pointer lock was unavailable | High | — |
| G2 | Explosions, chain reactions, decals and casings are implemented and wired but not visually confirmed | Medium-high | `effects/*`, `environment/Explosives.ts` |
